...
describe("experiment lifecycle tools", () => {
  ...
  it("appends a result row to autoresearch.jsonl and preserves secondary metrics in state", async () => {
    ...
    vi.spyOn(gitModule, "revertTrackedChanges").mockReturnValue({
      attempted: true,
      reverted: true,
  ...
  it("routes keep to commit and discard/crash to revert while preserving logged statuses", async () => {
    ...
    const revertSpy = vi.spyOn(gitModule, "revertTrackedChanges").mockReturnValue({
      attempted: true,
      reverted: true,
      summary: "Git: reverted tracked changes with git checkout -- .",
      command: {
